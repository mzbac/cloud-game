...
const keyboard = (() => {
  ...
  event.sub(KEYBOARD_TOGGLE_FILTER_MODE, (data) => {
    isKeysFilteredMode =
      data.mode !== undefined ? data.mode : !isKeysFilteredMode;
  ...
  function onDpadToggle(checked) {
    ...
    if (dpadMode) {
      ...
      value =
        (dpadState[KEY.RIGHT] === true ? 1 : 0) -
        (dpadState[KEY.LEFT] === true ? 1 : 0);
      if (value !== 0) {
        event.pub(AXIS_CHANGED, { id: 0, value: 0 });
      }
      value =
        (dpadState[KEY.DOWN] === true ? 1 : 0) -
        (dpadState[KEY.UP] === true ? 1 : 0);
      if (value !== 0) {
